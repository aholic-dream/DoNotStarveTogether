...
function RecipePopup:Refresh(...)
    ...
    --so this sucks, my only way to gain code execution where I need it is by replacing ipairs.
    ...
    local function ipairs_(t, ...)
        ipairs = _ipairs --this prevents infinite looping by making any calls to pairs inside the body of this function go to the original function, and not this one.
        if t == self.ing then
            --ipairs can get called an unknown amount of times before reaching where I need to actually inject my code
            --so I check for t == self.ing since that only gets called once, and right before the 3 ipairs loops I actually have to do stuff on.
            enabled = true
        elseif t == recipe.tech_ingredients then
            ...
            t = _tech_ingredients
            if enabled then
                --put tech_ingredients back after we reach the correct call to this ipairs loop
                recipe.tech_ingredients = _tech_ingredients
            end
        elseif t == recipe.ingredients and enabled then
            --obtain the index where recipe.ingredients start.
            startidx = #self.ing + 1
        elseif t == recipe.character_ingredients and enabled then
