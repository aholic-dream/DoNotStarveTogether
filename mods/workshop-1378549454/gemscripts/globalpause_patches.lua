...
if TheNet:GetIsServerAdmin() then
    ...
    function MapScreen:OnBecomeActive(...)
        if GLOBALPAUSE.AUTOPAUSEENABLED and GLOBALPAUSE.AUTOPAUSEMAP and GetPlayerCount() == 1 then
    ...
    function ConsoleScreen:OnBecomeActive(...)
        if GLOBALPAUSE.AUTOPAUSEENABLED and GLOBALPAUSE.AUTOPAUSECONSOLE and GetPlayerCount() == 1 then
            if not TheWorld.ispaused then
                TheWorld:SetPause(true)
                autopaused = true
            end
