...
DoubleLinkedList = Class(function(self, start_data, node_type, no_memory_management)
    node_type = node_type or NodeLink
    local first
    local last
    local length = 0

    local prox --prox must stay alive for the lifetime of this object, therefore it must stay in the scope of the entire object as an upvalue
    if not no_memory_management then
        prox = newproxy(true)
        getmetatable(prox).__gc = function() self:Destroy() end
    end
