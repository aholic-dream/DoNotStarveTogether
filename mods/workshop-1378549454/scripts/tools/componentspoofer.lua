...
function EntityScript:ReplicateSpoofedComponent(componentname, name)
	...
	self:AddTag("ActualyIs"..componentname)
	self:AddTag("NamedAs"..name)
...
local old_ReplicateEntity = EntityScript.ReplicateEntity
EntityScript.ReplicateEntity = function(self)
	local HasNamedComponent = false
	for k, v in pairs(COMPONENTS_REPLICATED_AS) do
		if self:HasTag("ActualyIs"..k) then
			HasNamedComponent = true
			break
		end
	end
	if HasNamedComponent then
	    for k, v in pairs(REPLICATABLE_COMPONENTS) do
	        if v and self:HasTag("ActualyIs"..k) then
	            self:ReplicateSpoofedComponent(k, COMPONENTS_REPLICATED_AS[k])
	        elseif v and (self:HasTag("_"..k) or self:HasTag("__"..k)) and not self:HasTag("NamedAs"..k) then
