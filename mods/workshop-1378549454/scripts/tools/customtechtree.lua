...
if TheNet:GetIsServer() then
    MODENV.AddComponentPostInit("builder", function(builder)
        local old_PushEvent = builder.inst.PushEvent
        function builder.inst:PushEvent(event, data)
            if event == "techtreechange" then
                ...
                old_PushEvent(self, event, {level = builder.accessible_tech_trees})
            else
                old_PushEvent(self, event, data)
            end
        end
    end)
...
MODENV.AddPrefabPostInit("player_classified", function(inst)
    for i, v in ipairs(CUSTOM_TECH_BONUS) do
