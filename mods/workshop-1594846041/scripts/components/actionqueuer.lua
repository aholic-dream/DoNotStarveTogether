...
local extend_deploy_names = {
	["trap_teeth"] = true,
	["minisign_item"] = true, -- 小木牌种植 shang
	["minisign_drawn"] = true, -- 已画的木牌 shang
	-- ["fossil_piece"] = true, -- 化石碎片 shang
}
...
--自动钓鱼
-- ACTIONS.HOOK = function(act)
--     local fishingrod = act.invobject.components.fishingrod
--     fishingrod:Hook()
--     if act.doer:HasTag("nibble") then
--         print("HOOK_001")
--         return "HOOK"
--     end
--     print("HOOK_002")
...
do
	...
	local allowed_actions = {
		[ACTIONS.REPAIR] = true,     --修理
		[ACTIONS.CHOP] = true,       --砍树
		[ACTIONS.COOK] = true,       --烹饪
		[ACTIONS.PICK] = true,       --采集
		[ACTIONS.PICKUP] = true,     --捡起
		[ACTIONS.MINE] = true,       --采矿
		[ACTIONS.DIG] = true,        --挖掘
		[ACTIONS.GIVE] = true,       --给予
		[ACTIONS.DRY] = true,        --晒干
		[ACTIONS.EXTINGUISH] = true, --熄灭
		[ACTIONS.BAIT] = true,       --诱饵
		[ACTIONS.CHECKTRAP] = true,  --检查陷阱
		[ACTIONS.HARVEST] = true,    --收获
		[ACTIONS.SHAVE] = true,      --剃刀
		[ACTIONS.NET] = true,        --捕捉
		[ACTIONS.FERTILIZE] = true,  --施肥
		[ACTIONS.HAMMER] = true,     --石锤
		[ACTIONS.RESETMINE] = true,  --重置
		[ACTIONS.ACTIVATE] = true,   --激活
		[ACTIONS.TURNON] = true,     --开启
		[ACTIONS.TURNOFF] = true,    --关闭
		[ACTIONS.USEITEM] = true,    --使用物品
		[ACTIONS.TAKEITEM] = true,   --拾取物品
		[ACTIONS.PLANT] = true,      --种植
		[ACTIONS.ADDFUEL] = true,    --添加燃料
		[ACTIONS.ADDWETFUEL] = true, --添加湿燃料
		[ACTIONS.DRAW] = true,       --自动描绘 Shang
		
		-- [ACTIONS.HOOK] = true,       --自动钓鱼2 Shang
		[ACTIONS.REEL] = true,       --自动钓鱼3 Shang
		[ACTIONS.FISH] = true,       --自动钓鱼1 Shang

		[ACTIONS.LIGHT] = true,      --批量点燃 Shang
		--[ACTIONS.] = true,         -- Shang
		-- [ACTIONS.STORE] = true,           --排队储存 Shang
		[ACTIONS.UNWRAP] = true,           --一键拆包 Shang
		[ACTIONS.HAUNT] = true,           --作祟成恶魔花 Shang
		--[] = true,           -- Shang
		--[] = true,           -- Shang
	}
	local special_cases = {}
	...
	special_cases[ACTIONS.HAUNT] = function(target, right)
		if right then
			return false
		end
		if target.prefab == "flower_evil" then 
			return false
		end
	...
	-- special_cases[ACTIONS.CASTSPELL] = disLeft --召唤一排星星 Shang
	special_cases[ACTIONS.HARVEST] = function(target, right) --鸟笼收鸟 Shang
		if right then
			return false
		end
