...
local function OnDurability(inst)
    if not inst:HasTag("burnt") and not inst.broken then
...
if inst.duration > 0 then
	...
	for i = 1, container:GetNumSlots() do
	     ...
	     if item and inst.light_on then
			if item:HasTag("musha_items") and item.level and item.level <4000 then 
					item.level = item.level +1
					inst.AnimState:PlayAnimation("hit")
		--SpawnPrefab("small_puff").Transform:SetPosition(inst:GetPosition():Get()) 
		SpawnPrefab("green_leaves").Transform:SetPosition(inst:GetPosition():Get()) 
				if inst.duration > 0 then
					inst.duration = inst.duration - 0.25
				elseif inst.duration <= 0 then	
...
elseif inst.duration <= 0 and not inst.broken then	
if not inst:HasTag("burnt") then
	inst.broken = true
inst.Light:Enable(false)
inst.light_on = false
